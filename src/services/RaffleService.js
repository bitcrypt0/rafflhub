...
class RaffleService {
  ...
  /**
   * Fetch minimal raffle metadata for filtering (mobile optimization)
   */
  async fetchRaffleMetadata(raffleAddress, config = {}) {
    try {
      const raffleContract = this.createContractInstance(raffleAddress, 'raffle', true);
      if (!raffleContract) {
        console.error(`Failed to create raffle metadata contract for ${raffleAddress}`);
        return null;
      }

      // Fetch only essential fields needed for filtering (much faster)
      const [stateNum, isPrizedContract, prizeCollection, standard, isExternallyPrized] = await Promise.all([
      ...
      // Map state number to string for filtering
      let raffleState;
      switch (stateNum) {
      ...
      return {
        ...
        // Minimal data for filtering only
        name: `Raffle ${raffleAddress.slice(0, 8)}...`, // Placeholder name
        creator: ethers.constants.AddressZero, // Placeholder
        ticketPrice: ethers.BigNumber.from(0), // Placeholder
        erc20PrizeAmount: ethers.BigNumber.from(0), // Placeholder
        nativePrizeAmount: ethers.BigNumber.from(0) // Placeholder
      };
    } catch (error) {
      console.error(`Error fetching raffle metadata for ${raffleAddress}:`, error);
      return null;
    }
  }
  ...
  /**
   * Fetch metadata for all raffles (mobile optimization for filtering)
   */
  async fetchAllRaffleMetadata(options = {}) {
    const {
      isMobile = false,
      useCache = true
    } = options;
    ...
    const cacheKey = `allRaffleMetadata_${this.walletContext?.chainId}_${isMobile}`;
    const timerKey = this.startTimer('fetchAllRaffleMetadata', { isMobile, chainId: this.walletContext?.chainId });

    // Check cache first
    if (useCache) {
      const cached = this.cache.get(cacheKey);
      if (cached && Date.now() - cached.timestamp < this.cacheTimeout) {
        this.endTimer(timerKey, { fromCache: true, metadataCount: cached.data.length });
        return cached.data;
      }
    }
    ...
    try {
      ...
      if (!allAddresses || allAddresses.length === 0) {
        this.endTimer(timerKey, { metadataCount: 0, fromCache: false });
        return [];
      }

      console.log(`[RaffleService] Fetching metadata for ${allAddresses.length} raffles (mobile optimization)`);

      // Fetch metadata in optimized batches for mobile
      const batchSize = isMobile ? 3 : 6; // Smaller batches for mobile
      const results = [];
      ...
      for (let i = 0; i < allAddresses.length; i += batchSize) {
        ...
        try {
          const batchPromises = batch.map(address =>
            this.fetchRaffleMetadata(address, { isMobile })
          );
          ...
          // Progress logging
          console.log(`[RaffleService] Metadata progress: ${results.length}/${allAddresses.length} (${((results.length/allAddresses.length)*100).toFixed(1)}%)`);

          // Small delay between batches on mobile
          if (i + batchSize < allAddresses.length && isMobile) {
      ...
      console.log(`[RaffleService] Successfully fetched metadata for ${results.length}/${allAddresses.length} raffles`);
      return results;
    } catch (error) {
      this.trackError('fetchAllRaffleMetadata', error, { isMobile, chainId: this.walletContext?.chainId });
  ...
  /**
   * Progressive loading for mobile - fetch raffle details in batches with callbacks
   */
  async fetchRaffleDetailsProgressively(addresses, options = {}) {
    const {
      isMobile = false,
    ...
    console.log(`[RaffleService] Starting progressive loading for ${addresses.length} raffles (mobile: ${isMobile})`);

    for (let i = 0; i < addresses.length; i += batchSize) {
      const batch = addresses.slice(i, i + batchSize);

      try {
        // For mobile, use smaller parallel batches with better error handling
        const batchPromises = batch.map(address =>
          this.fetchRaffleDetails(address, { isMobile })
        );
        ...
        if (onProgress) {
          onProgress(results.length, addresses.length, successfulResults);
        }

        // Call batch complete callback
        if (onBatchComplete) {
          onBatchComplete(successfulResults, i / batchSize + 1, Math.ceil(addresses.length / batchSize));
        }

        console.log(`[RaffleService] Progressive batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(addresses.length / batchSize)} complete: ${results.length}/${addresses.length} total`);

        // Delay between batches for mobile
        if (i + batchSize < addresses.length && isMobile) {
          await new Promise(resolve => setTimeout(resolve, config.batchDelay));
        }
      } catch (error) {
        console.error(`Error in progressive batch ${Math.floor(i / batchSize) + 1}:`, error);
        // Continue with next batch
      }
    }

    console.log(`[RaffleService] Progressive loading complete: ${results.length}/${addresses.length} raffles loaded`);
    return results;
  }
