...
import { Gift, Info, FileText, Clock, Users, Shield, Share2, Loader2, Check, CheckCircle2, ExternalLink, Sparkles } from 'lucide-react'
...
/**
 * LuckySaleERC721Form - Create a lucky sale raffle with escrowed ERC721
 * Raffle off an existing NFT from your wallet
 */
function LuckySaleERC721Form({ onReviewStateChange }) {
  ...
  const validateSection = (section) => {
    ...
    switch (section) {
      ...
      case 'participation':
        ...
        if (!formData.maxTicketsPerParticipant) newErrors.maxTicketsPerParticipant = 'Max tickets per participant is required'
        break
    }
  ...
  useEffect(() => {
    setSectionCompletion({
      basic: !!formData.name?.trim() && !!formData.collectionAddress?.trim() && !!formData.tokenId,
      timing: !!formData.startTime && !!formData.duration,
      participation: !!formData.slotLimit && !!formData.winnersCount && !!formData.maxTicketsPerParticipant,
    })
  }, [formData])
  ...
  const summaryData = [
    ...
    { label: 'Prize Collection', value: formData.collectionAddress || 'Not set', type: 'text' },
    { label: 'Prize Token ID', value: formData.tokenId || 'Not set', type: 'text' },
    ...
    { label: 'Winners', value: formData.winnersCount || '1', type: 'prize' },
      ...
      <Card variant="elevated" className="p-8 max-w-3xl mx-auto text-center">
        ...
        >
          <div className="mb-6">
            ...
            <p className="text-muted-foreground">Your NFT lucky sale pool has been deployed to the blockchain.</p>
          </div>
  ...
  return (
    <Card variant="elevated" className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        <h3 className="font-display text-xl font-semibold">NFT Lucky Sale Event</h3>
        <p className="text-sm text-muted-foreground">Give away an existing NFT from your wallet to a lucky winner</p>
      </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleReview(); }} className="space-y-4">
        ...
        <FormSection
          ...
          hasError={!!errors.name || !!errors.collectionAddress || !!errors.tokenId}
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="My NFT Lucky Sale"
                required
              />
            </div>

            <div>
              <label className="block font-body text-sm font-medium mb-2">Prize Collection Address *</label>
              <input
                type="text"
        ...
        <FormSection
          ...
          hasError={!!errors.slotLimit || !!errors.winnersCount || !!errors.maxTicketsPerParticipant}
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block font-body text-sm font-medium mb-2 flex items-center gap-2">
            ...
            <div>
              <label className="block font-body text-sm font-medium mb-2 flex items-center gap-2">
                ...
                <Tooltip>
                  ...
                  <TooltipContent sideOffset={6}>
                    This pool type supports only one winner
                  </TooltipContent>
                </Tooltip>
              </label>
              <input
...
export default LuckySaleERC721Form
