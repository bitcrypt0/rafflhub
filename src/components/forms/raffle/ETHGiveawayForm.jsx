...
/**
 * ETHGiveawayForm - Create a native token (ETH/AVAX) giveaway
 * Give away native tokens to your community
 */
function ETHGiveawayForm({ onReviewStateChange }) {
  ...
  const validateSection = (section) => {
    ...
    switch (section) {
      case 'basic':
        ...
        if (!formData.ethAmount) newErrors.ethAmount = 'Prize amount is required'
        break
      case 'timing':
  ...
  useEffect(() => {
    setSectionCompletion({
      basic: !!formData.name?.trim() && !!formData.ethAmount,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const ethAmount = formData.ethAmount
        ? ethers.utils.parseEther(formData.ethAmount)
  ...
  const summaryData = [
    ...
    { label: 'Prize Amount', value: formData.ethAmount ? `${formData.ethAmount} ${getCurrencyLabel()}` : 'Not set', type: 'currency' },
      ...
      <Card variant="elevated" className="p-8 max-w-3xl mx-auto text-center">
        ...
        >
          <div className="mb-6">
            ...
            <p className="text-muted-foreground">Your {getCurrencyLabel()} giveaway raffle has been deployed to the blockchain.</p>
          </div>
  ...
  return (
    <Card variant="elevated" className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        <h3 className="font-display text-xl font-semibold">{getCurrencyLabel()} Giveaway</h3>
        <p className="text-sm text-muted-foreground">Give away native tokens to your community</p>
      </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleReview(); }} className="space-y-4">
        ...
        <FormSection
          ...
          hasError={!!errors.name || !!errors.ethAmount}
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block font-body text-sm font-medium mb-2">{getCurrencyLabel('prize')} *</label>
              <input
                type="number"
                min="0.00000001"
                step="any"
...
export default ETHGiveawayForm
